...
final class SocketIOManager {
    ...
    private let baseURL = URL(string: PrivateKey.baseURL.rawValue + "/v1")!
    var roomID = ""
    private lazy var forNamespace = PrivateKey.socketKeyPath.rawValue + roomID
    
    let receivedChatData = PublishSubject<ChatResponse.ChatData>()
        ...
        socket.on("chat") { dataArray, ack in
            ...
            if let data = dataArray.first {
                do {
                    ...
                    let decodedData = try JSONDecoder().decode(ChatResponse.ChatData.self, from: result)
